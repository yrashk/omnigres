...
return encode(
			digest(method || chr(10) 
				|| uri || chr(10) 
				|| query || chr(10) 
				|| array_to_string(headers, chr(10)) || chr(10) || chr(10) 
				|| array_to_string(sheaders, ';'::text) || chr(10)
				|| payload_hash, 
			'sha256'),
		'hex');

create or replace function hash_string_to_sign(
												algorithm text,
...
return encode(hmac((((((((
						algorithm || chr(10) 
						|| to_char((ts8601 AT TIME ZONE 'UTC'::text), 'YYYYMMDD"T"HH24MISS"Z"'::text) || chr(10) 
						|| to_char((ts8601 AT TIME ZONE 'UTC'::text), 'YYYYMMDD'::text)) || '/'
						|| region) || '/') || service) || '/aws4_request') || chr(10))
						|| canonical_request_hash)::bytea,
						hmac('aws4_request', 
							hmac(service::bytea, 
								hmac(region::bytea, 
									hmac(to_char((ts8601 AT TIME ZONE 'UTC'::text), 'YYYYMMDD'::text)::bytea, 
										('AWS4' || secret_access_key)::bytea, 
									'SHA256'::text)::bytea, 
								'SHA256'::text)::bytea, 
						'SHA256'::text)::bytea, 
					'SHA256'::text)::bytea, 
...
create or replace function list_objects_v2(
											region text,
											service text,
											bucket text,
											secret_access_key text,
											...
											ts8601 timestamp with time zone)
returns text as $$
	select convert_from(body, 'utf-8')
	from omni_httpc.http_execute(
			omni_httpc.http_request('https://'||bucket||'.'||service||'.'||region||'.amazonaws.com' || uri || query, 
				headers => array[
					...
					omni_http.http_header(
						'X-Amz-Date', 
						to_char((ts8601 AT TIME ZONE 'UTC'::text), 'YYYYMMDD"T"HH24MISS"Z"'::text)),
					omni_http.http_header(
						'Authorization', 
