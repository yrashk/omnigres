...
create or replace function transaction_affected_accounts()
    returns table
            (
...
create or replace function statement_affected_accounts()
    returns table
            (
                account_id account_id,
                ledger_id  ledger_id
...
create or replace function deny_operation_on_immutable_table() returns trigger as
$$
begin
...
create or replace function deny_operation_on_accounts() returns trigger
    language c as
'MODULE_PATHNAME';

create or replace function calculate_account_balances() returns trigger
    language c as
...
create or replace function update_account_balances() returns trigger
    language c as
'MODULE_PATHNAME' stable;
