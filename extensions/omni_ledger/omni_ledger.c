...
static TransactionId previously_observed_transaction = InvalidTransactionId;
...
static inline bool invalidated() {
  ...
  if (!TransactionIdIsValid(current) || current == previously_observed_transaction) {
    // No new transaction yet
    return false;
  }
  previously_observed_transaction = current;
  return TransactionIdFollowsOrEquals(*invalidation, current);
}
