...
begin
    ...
    url := resources_url(group_version, resource, namespace => case when is_namespaced then '%s' end);
    ...
    create function %1$I() returns text return %2$I.url_with(%2$I.resources_url(%3$L, %4$L), label_selector => %5$L, field_selector => %6$L)
    $table_resource_$, table_name || '_resource', ns, group_version, resource, label_selector,
                   field_selector);
